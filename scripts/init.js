    ...
    Fluxion.doAjax(
        ...
        {'query': ref, 'url': ajaxurl},
        {
            'doOnSuccess': function (json) {
                ...
                var content = "<table class='list' id='search_hit' ><thead><tr><th>Genome_db_id</th><th>Genome</th></tr></thead>";

                for (var i = 0; i < json.result.length; i++) {

                    content += "<tr onclick=\"jQuery('#searchresult').fadeOut(); jQuery('#canvas').fadeIn();changeGenome('" + json.result[i].id + "','" + json.result[i].name.replace(/\s+/g, "_") + "'), getChromosomes()\"><td> " + json.result[i].id + "<td>" + json.result[i].name;
                    }
                    content += "</table>";
                    jQuery("#searchresult").html(content);
                    jQuery("#searchresult").fadeIn();
                    jQuery("#canvas").fadeOut();
